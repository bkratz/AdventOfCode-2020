package de.birgitkratz.aoc2020.day07;

import java.io.IOException;
import java.util.List;
import java.util.stream.Collectors;

import org.junit.jupiter.api.Test;

import static org.assertj.core.api.Assertions.assertThat;

class HandyHaversacksTest {

    HandyHaversacks handyHaversacks = new HandyHaversacks();

    @Test
    void processPart1() throws IOException {
        assertThat(handyHaversacks.processPart1()).isEqualTo(119);
    }

    @Test
    void processPart2() throws IOException {
        assertThat(handyHaversacks.processPart2()).isEqualTo(155802);
    }

    @Test
    void processInputPart1() {
        String input = "light red bags contain 1 bright white bag, 2 muted yellow bags.\n"
                + "dark orange bags contain 3 bright white bags, 4 muted yellow bags.\n"
                + "bright white bags contain 1 shiny gold bag.\n"
                + "muted yellow bags contain 2 shiny gold bags, 9 faded blue bags.\n"
                + "shiny gold bags contain 1 dark olive bag, 2 vibrant plum bags.\n"
                + "dark olive bags contain 3 faded blue bags, 4 dotted black bags.\n"
                + "vibrant plum bags contain 5 faded blue bags, 6 dotted black bags.\n"
                + "faded blue bags contain no other bags.\n"
                + "dotted black bags contain no other bags.";

        final List<String> inputList = input.lines().collect(Collectors.toList());

        assertThat(handyHaversacks.processInputPart1(inputList)).isEqualTo(4);
    }

    @Test
    void processInputPart2_1() {
        String input = "light red bags contain 1 bright white bag, 2 muted yellow bags.\n"
                + "dark orange bags contain 3 bright white bags, 4 muted yellow bags.\n"
                + "bright white bags contain 1 shiny gold bag.\n"
                + "muted yellow bags contain 2 shiny gold bags, 9 faded blue bags.\n"
                + "shiny gold bags contain 1 dark olive bag, 2 vibrant plum bags.\n"
                + "dark olive bags contain 3 faded blue bags, 4 dotted black bags.\n"
                + "vibrant plum bags contain 5 faded blue bags, 6 dotted black bags.\n"
                + "faded blue bags contain no other bags.\n"
                + "dotted black bags contain no other bags.";

        final List<String> inputList = input.lines().collect(Collectors.toList());

        assertThat(handyHaversacks.processInputPart2(inputList)).isEqualTo(32);
    }

    @Test
    void processInputPart2_2() {
        String input = "shiny gold bags contain 2 dark red bags.\n"
                + "dark red bags contain 2 dark orange bags.\n"
                + "dark orange bags contain 2 dark yellow bags.\n"
                + "dark yellow bags contain 2 dark green bags.\n"
                + "dark green bags contain 2 dark blue bags.\n"
                + "dark blue bags contain 2 dark violet bags.\n"
                + "dark violet bags contain no other bags.";

        final List<String> inputList = input.lines().collect(Collectors.toList());

        assertThat(handyHaversacks.processInputPart2(inputList)).isEqualTo(126);
    }

}